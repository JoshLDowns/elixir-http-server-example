defmodule Servy.HttpServer do
  ...
  def start(port) when is_integer(port) and port > 1023 do
    ...
    # Creats a socket to listen for client connections.
    ...
    # Socket options (don't worry about these details specifically, many different use cases):
  ...
  def accept_loop(listen_socket) do
    ...
    # is accepted, `client_socket` is boud to a new client socket.
    {:ok, client_socket} = :gen_tcp.accept(listen_socket)

    IO.puts "⚡️  Connection accepted!\n"

    # Receives the request and sends a response over the client socket.
    # spawn calls serve asynchronously so the loop can continue and not break on long calls
